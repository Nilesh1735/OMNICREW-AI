...
const PipelineGraph: React.FC<PipelineGraphProps> = ({ logs, isProcessing }) => {
  ...
  // Hardcoded LLM tracking logic for DeepSeek -> Mistral -> Gemini
  const isDeepSeekActive = logs.some(l => l.thought_process?.toLowerCase().includes('deepseek') && l.action === 'LLM Routing');
  const isDeepSeekFailed = logs.some(l => l.action === 'LLM Fallback') && logs.some(l => l.thought_process?.toLowerCase().includes('deepseek'));
  ...
  const nodes: Node[] = useMemo(() => [
    ...
    { id: '5', data: { label: 'DeepSeek' }, position: { x: 350, y: -50 }, className: `custom-node llm-node ${isDeepSeekActive ? 'node-breathing' : ''} ${isDeepSeekFailed ? 'node-failed' : ''}` },
  ...
  ], [isResearcherActive, isAnalystActive, isDeepSeekActive, isDeepSeekFailed, isMistralActive, isMistralFailed, isGeminiActive]);
  ...
  const edges: Edge[] = useMemo(() => [
    ...
    { id: 'e2-5', source: '2', target: '5', animated: isDeepSeekActive, className: 'custom-edge' },
    { id: 'e5-6', source: '5', target: '6', animated: isDeepSeekFailed, className: 'custom-edge' },
    { id: 'e6-7', source: '6', target: '7', animated: isMistralFailed, className: 'custom-edge' },
    { id: 'e5-3', source: '5', target: '3', animated: isDeepSeekActive, className: 'custom-edge' },
  ...
  ], [isProcessing, isResearcherActive, isAnalystActive, isDeepSeekActive, isDeepSeekFailed, isMistralActive, isMistralFailed, isGeminiActive]);
